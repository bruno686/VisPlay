...
from jinja2 import Template          # already used in _build_messages
import torch
...
def save_responses_for_validation(batch, tokenizer, output_path):
    ...
    input_ids    = batch.batch["input_ids"]       # shape: [B, seq_len]
    response_ids = batch.batch["responses"]       # shape: [B, seq_len]
    batch_size   = len(batch)                     # = B
    ...
    # dataset_index should be length B
    dataset_indices = batch.non_tensor_batch["dataset_index"]
    if isinstance(dataset_indices, np.ndarray):
        dataset_indices = dataset_indices.tolist()

    # decode each
    prompts   = [tokenizer.decode(ids, skip_special_tokens=True)
                 for ids in input_ids]
    responses = [tokenizer.decode(ids, skip_special_tokens=True)
                 for ids in response_ids]

    print(f"Saving to {output_path}…")
    with open(output_path, "a", encoding="utf-8") as f:
        for idx, (orig_idx, prompt, response) in enumerate(
            zip(dataset_indices, prompts, responses)
        ):
            f.write(json.dumps({
                # "index": idx,
...
class RayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        '''
        Save the validation answers
        '''
        if self.config.trainer.val_only:
            # resp_path = self.config.trainer.response_path
        ...
        for batch_dict in self.val_dataloader:
            ...
            '''
            Below is added code for test batch
            '''
            _ensure_numpy_arrays(test_batch.non_tensor_batch)
            _ensure_numpy_arrays(test_output_gen_batch.non_tensor_batch)
            '''
            Above is added code for test batch
            '''
            ...
            if self.config.trainer.val_only:
                save_responses_for_validation(
                    test_batch,                 # ← RIGHT: has prompts + responses (after union)
                    self.tokenizer,
                    resp_path
                )
        ...
        for _ in tqdm(range(self.config.trainer.total_epochs), desc="Epoch", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with timer("step", timing_raw):
                    ...
                    '''
                    Below is the code to generate second hop outputs
                    '''    
                    
                    # ----------------------------------------------------------------------
                    # DEBUG: 2ʳᵈ-round prompt built with the SAME template & post-processing
                    # ----------------------------------------------------------------------
                    ...
                    with torch.no_grad():
                        ...
                        # print(f"[step {self.global_step}]  1st-round ⇒ {resp_texts[0]}\n")

                        # ── pull shared settings from the dataset instead of self.*
                        ds              = self.train_dataloader.dataset          # PromptDataset
                        max_len         = getattr(ds, "max_prompt_length", self.tokenizer.model_max_length)
                        truncation_mode = getattr(ds, "truncation", "right")
                        
                        
                        # ------------------------------------------------------------
                        # fetch original questions (length B)
                        # ------------------------------------------------------------
                        questions = batch.non_tensor_batch["question"].tolist()   # → list[str]
                        B          = len(questions)
                        n          = self.config.worker.rollout.n                 # e.g. 8

                        # ------------------------------------------------------------
                        # align to resp_texts (length B×n or B when n==1)
                        # ------------------------------------------------------------
                        if n > 1:
                            if len(resp_texts) != B * n:
                                raise RuntimeError(
                                    f"Mismatch: got {len(resp_texts)} responses, expected {B*n} (B={B}, n={n})."
                                )
                            questions = [q for q in questions for _ in range(n)]  # repeat each q n times

                        # print("Lengths equal?", len(questions) == len(resp_texts))
                        # print("Expanded question-list len:", len(questions))
                        ...
                        # In here, I need help for accessing the question in the data and create second input
                        ...
                        # 3) tokenise & pad right
                        model_inputs = self.tokenizer(prompts, add_special_tokens=False,
                                                    padding=True, return_tensors="pt")
                        input_ids      = model_inputs.pop("input_ids")
                        attention_mask = model_inputs.pop("attention_mask")

                        # 4) position_ids then **left-pad & truncate** identically to dataset
                        ...
                        # 5) truncate raw_ids_list the same way
                        for i, ids in enumerate(raw_ids_list):
                            if len(ids) > max_len:
                                if truncation_mode == "left":
                                    raw_ids_list[i] = ids[-max_len:]
                                elif truncation_mode == "right":
                                    raw_ids_list[i] = ids[:max_len]
                                else:
                                    raise RuntimeError(
                                        f"Prompt len {len(ids)} > {max_len} with truncation='error'"
                                    )
                        raw_prompt_ids = np.array(raw_ids_list, dtype=object)  # stays in non-tensor dict

                        # ----------------------------------------------------------------------
                        # ❶ Second-hop generation – keep exactly ONE response per prompt
                        # ----------------------------------------------------------------------
                        ...
                        second_out = self.actor_rollout_wg.generate_sequences(second_gen_batch)

                        sec_ids = second_out.batch["responses"]
                        if sec_ids.dim() == 3:                       # (B·n, 1, L) ➜ (B·n, L)
                            sec_ids = sec_ids.view(-1, sec_ids.size(-1))

                        second_texts = self.tokenizer.batch_decode(
                            sec_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False
                        )
                        # ----------------------------------------------------------------------
                        # DO NOT touch batch here (no description_answers yet, no union yet)
                        # ----------------------------------------------------------------------

                        # batch = batch.repeat(repeat_times=self.config.worker.rollout.n, interleave=True)

                        # add the list decoded earlier; length = B × n, so it matches the new batch size
                        # batch.non_tensor_batch["description_answers"] = np.array(second_texts, dtype=object)

                        # batch = batch.union(gen_batch_output)   # ← keep the original union
                    # ----------------------------------------------------------------------
                    
                         
                    '''
                    Above is the code to generate second hop outputs
                    '''
                    ...
                    '''
                    batch.non_tensor_batch["description_answers"] = np.array(
                        second_texts, dtype=object
                    )
                    
                    batch = batch.union(gen_batch_output)
                    n_responses = self.config.worker.rollout.n

                    # print('Batch is: ', batch)
                    # print("responses" in batch.batch.keys(), batch.batch.keys())
                    # print("input_ids" in batch.batch.keys(), batch.batch.keys())
