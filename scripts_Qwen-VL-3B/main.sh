...
Base_model=yourpath/llm/Qwen2.5-VL-3B-Instruct
...
bash scripts_Qwen-VL-3B/solver_train.sh $Base_model ${STORAGE_PATH}/models/${Model_abbr}_questioner_v1/global_step_40/actor/huggingface ${Model_abbr}_solver_v1
...
for i in {4..5}; do
    ...
    if [ -d "${STORAGE_PATH}/models/${Model_abbr}_questioner_v${i}/global_step_40/actor/huggingface" ]; then
        ...
        bash scripts_Qwen-VL-3B/questioner_train.sh \
            ...
            ${STORAGE_PATH}/models/${Model_abbr}_questioner_v${prev}/global_step_40/actor/huggingface \
            ${Model_abbr}_questioner_v${i}
    fi

    # Check if solver model already exists
    if [ -d "${STORAGE_PATH}/models/${Model_abbr}_solver_v${i}/global_step_40/actor/huggingface" ]; then
        echo "Solver model ${Model_abbr}_solver_v${i} already exists, skipping training..."
    else
        # Train solver
        bash scripts_Qwen-VL-3B/solver_train.sh \
            ${STORAGE_PATH}/models/${Model_abbr}_solver_v${prev}/global_step_40/actor/huggingface \
            ${STORAGE_PATH}/models/${Model_abbr}_questioner_v${i}/global_step_40/actor/huggingface \
